...
// ===========================================
const conteneur_apiliste = document.querySelector('#api-liste');

async function chargerPays() {

  conteneur_apiliste.innerHTML = '<p class="loading">Chargement des pays</p>';
  ...
  try {
    ...
    pays.forEach((p) => {
      ...
      `;
      conteneur_apiliste.append(carte);
    });

  } catch (erreur) {
    conteneur_apiliste.innerHTML = `<p class="error">Impossible de charger les pays:  ${erreur.message} </p>`;
    console.error(erreur);
  }
}
...
// ===========================================
const conteneur_recherche_resultat = document.querySelector('#recherche-resultats');
...
input.addEventListener('input', async () => {
  ...
  if (terme.length < 2) {
    conteneur_recherche_resultat.innerHTML = '<p>Tapez au moins 2 caractères.</p>';
    return;
  }

  conteneur_recherche_resultat.innerHTML = '<p class="loading">Recherche...</p>';
  ...
  try {
    ...
    conteneur_recherche_resultat.innerHTML = '';
    pays.forEach((p) => {
      const carte = document.createElement('article');
      carte.classList.add('carte');
      carte.innerHTML = `
        ...
        <p>Population : ${p.population.toLocaleString()}</p>
      `;
      conteneur_recherche_resultat.append(carte);
    });
    
  } catch (erreur) {
    conteneur_recherche_resultat.innerHTML = '<p class="error">Erreur de recherche.</p>';
  }
});
